	...
	<div class="about-container bg-background">
	  ...
	  <section class="cta-section bg-primary text-primary-foreground">
		<div class="container mx-auto px-4 py-16 text-center">
		  {#if slice.primary?.cta_title && isFilled.richText(slice.primary.cta_title)}
			<h2 class="text-4xl font-bold text-white mb-4">
			  <PrismicRichText field={slice.primary.cta_title} />
			</h2>
		  {/if}
		  {#if slice.primary?.cta_description && isFilled.richText(slice.primary.cta_description)}
			<div class="text-xl text-white mb-8">
			  <PrismicRichText field={slice.primary.cta_description} />
			</div>
		  {/if}
		  {#if slice.primary?.cta_button_link && isFilled.link(slice.primary.cta_button_link)}
			<PrismicLink field={slice.primary.cta_button_link}>
			  <Button variant="outline" size="lg" class="text-white border-white hover:bg-white hover:text-primary">
				{slice.primary?.cta_button_text || 'Book Your Free Consultation'}
			  </Button>
			</PrismicLink>
		  {/if}
		</div>
